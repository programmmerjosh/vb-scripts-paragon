...
Sub GetOuterSummary()
    ...
    If Is2DArrayEmpty(summaryData) Then
        ...
        wsFilteredData.Columns(11).Delete ' delete (11)OUTERS column because we have no mapped outers in this case
    Else
    ...
    ' UPDATE COLUMN NAME
    wsFilteredData.Cells(1, mailProviderCol.Column).Value = "MAIL"
    ...
    ' */
    Dim arrLatestWorkOrders As Variant, arrPreviousWorkOrders As Variant, arrPreviousParent As Variant, arrPreviousCorp As Variant
    ...
    If SheetExists("previous") Then
        ...
        arrPreviousParent = GetWorkUnitArray(wsPreviousFilteredData, "PARENT_NM", lastRowPreviousDataset)
        arrPreviousCorp = GetWorkUnitArray(wsPreviousFilteredData, "CORP_CD", lastRowPreviousDataset)
    ...
    Call AppendMissingWorkUnits(wsFilteredData, arrPreviousParent, arrPreviousCorp, arrPreviousWorkOrders, arrLatestWorkOrders, summaryEndRow)
...
Sub GetPrintCompleted()
    ...
    keepList = Array("PARENT_NM", "CORP_CD", "TARGET_MAIL_DATE", "WORK_UNIT_CD", "STMT_CNT", "REM_MC_CNT", "REM_MC2_CNT", "PRT_END", "PRT_REM_END")

    Call CopyColumnsByHeader(wsSpecial, wsFilteredData, keepList)
    Call SortByColumn(wsFilteredData, "WORK_UNIT_CD")
    
    Dim datasetCORPCol As Range
    Dim lastRowDataset As Long
    ...
    ' Sub HighlightRowForCondition(ws As Worksheet, highlightCol As String, highlightColor As Long, condition As String, forCondition As Boolean = True)
    Call HighlightRowForCondition(wsFilteredData, "REM_MC_CNT", RGB(255, 255, 0), "", False) ' cYellow
    Call FormatFilteredDataSheet2(wsFilteredData, lastRowDataset)
...
End Sub
' === Supporting Functions Of GetOuterSummary() [START] ===
...
Sub HighlightAlwaysOrderedOuters(ws As Worksheet, outerColIndex As Long, colNumToHighlight As Long, highlightColor As Variant)
    ...
    outersToOrder = Array("50023", "BSMAIR", "BARC5W(1024)", "50011C5TNT0716")
...
Sub FormatFilteredDataSheet(ws As Worksheet, lastRow As Long)
    With ws
        ...
        .Columns(10).HorizontalAlignment = xlLeft
        .Columns(11).HorizontalAlignment = xlLeft

        ' Set column widths (A to H)
        .Columns("A").ColumnWidth = 8 ' client
        .Columns("B").ColumnWidth = 8 ' corp
        .Columns("C").ColumnWidth = 7 ' work order
        .Columns("D").ColumnWidth = 8.5 ' pack count
        .Columns("E").ColumnWidth = 6.5 ' inserts
        .Columns("F").ColumnWidth = 6.5 ' rem
        .Columns("G").ColumnWidth = 2 ' plan
        .Columns("H").ColumnWidth = 2 ' mail
        ...
        With .Range(.Cells(1, 1), .Cells(lastRow, 11)).Borders
...
Sub FormatFilteredDataSheet2(ws As Worksheet, lastRow As Long)
    With ws
        ' Align column H (column 8)
        ...
        ' Set column widths (A to H)
        .Columns("A").ColumnWidth = 20 ' client
        .Columns("B").ColumnWidth = 7 ' corp
        .Columns("C").ColumnWidth = 16 ' target mail date
        .Columns("D").ColumnWidth = 6.5 ' work unit
        .Columns("E").ColumnWidth = 6.5 ' pack count
        .Columns("F").ColumnWidth = 4.5 ' rem count
        .Columns("G").ColumnWidth = 4 ' rem 2 count
        .Columns("H").ColumnWidth = 9 ' print end
        .Columns("I").ColumnWidth = 9 ' print rem end
        ...
        ' Apply borders to data
        With .Range(.Cells(1, 1), .Cells(lastRow, 9)).Borders
            .LineStyle = xlContinuous
            .Color = vbBlack
            .Weight = xlThin
        End With
...
' For SIDE STEP between 6 & 7 (1 of 2) & MergeBeginSummary()
...
' For STEP 7 (1 of 2) & MergeBeginSummary()
...
' For STEP 8
Sub AppendMissingWorkUnits( _
    ws As Worksheet, _
    arrPreviousParent As Variant, _
    arrPreviousCorp As Variant, _
    arrPreviousWorkOrder As Variant, _
    arrLatestWorkOrder As Variant, _
    summaryEndRow As Long)

    Dim missingParentValues() As Variant
    Dim missingCorpValues() As Variant
    Dim missingWorkOrderValues() As Variant
    ...
    ' Compare each previous work order against latest work orders
    For i = 1 To UBound(arrPreviousWorkOrder, 1)
        foundMissing = True

        For j = 1 To UBound(arrLatestWorkOrder, 1)
            If arrPreviousWorkOrder(i, 1) = arrLatestWorkOrder(j, 1) Then
                foundMissing = False
                Exit For
            End If
        Next j
        ...
        If foundMissing Then
            ...
            ReDim Preserve missingParentValues(1 To missingCount)
            ReDim Preserve missingCorpValues(1 To missingCount)
            ReDim Preserve missingWorkOrderValues(1 To missingCount)
            
            missingParentValues(missingCount) = arrPreviousParent(i, 1)       ' PARENT_NM
            missingCorpValues(missingCount) = arrPreviousCorp(i, 1)         ' CORP_CD
            missingWorkOrderValues(missingCount) = arrPreviousWorkOrder(i, 1)    ' STMT_CN
        End If
    Next i
    ...
    If missingCount > 0 Then
        ...
        ' Heading
        ws.Cells(startRow, 1).Value = "ENCLOSED JOBS"
        ws.Cells(startRow, 1).Font.Bold = True

        ' Column headers
        ws.Cells(startRow + 1, 1).Value = "PARENT_NM"
        ws.Cells(startRow + 1, 2).Value = "CORP_CD"
        ws.Cells(startRow + 1, 3).Value = "STMT_CN"
        ws.Range(ws.Cells(startRow + 1, 1), ws.Cells(startRow + 1, 3)).Font.Bold = True

        ' Data rows
        For i = 1 To missingCount
            ws.Cells(startRow + i, 1).Value = missingParentValues(i)
            ws.Cells(startRow + i, 2).Value = missingCorpValues(i)
            ws.Cells(startRow + i, 3).Value = "'" & missingWorkOrderValues(i) ' Apostrophe for string preservation
        Next i

        ' Format the output range
        With ws.Range(ws.Cells(startRow, 1), ws.Cells(startRow + missingCount, 3))
            .Font.Italic = True
...
' === Supporting Functions Of GetOuterSummary() [END] ===

'=======================
'   Module: MergeTool
'=======================

Sub MergeBeginSummary()
    ...
    Call GetOuterSummary
End Sub

Sub MergeBeginPrintCompleted()
    Dim wsTarget As Worksheet
    Dim wb As Workbook
    Dim sheetNames As Variant
    Dim i As Integer
    ...
    Call GetPrintCompleted
End Sub

' === Supporting Functions Of MergeBeginSummary() [START] ===
...
Sub FormatWorkUnitColumn(ws As Worksheet, startRow As Long, endRow As Long, colNum As Long)
    ...
    End With
End Sub
' === Supporting Functions Of MergeBeginSummary() [END] ===
...
Sub SortByClient()
    ...
    Dim sortRange As Range
    Set sortRange = ws.Range(ws.Cells(1, 1), ws.Cells(lastRow, 10)) ' Col A to I, unmerged
...
' Sort our Enclosed worksheet by client
Sub SortEnclosedJobsByClient()
    Dim ws As Worksheet
    Dim col As Range
    Dim lastRow As Long
    Dim wsEnclosed As Worksheet

    On Error Resume Next
    Set ws = ThisWorkbook.Sheets("Enclosed")
    On Error GoTo 0
    ...
    Set wsEnclosed = ThisWorkbook.Sheets("Enclosed")
    Call AddTimestampToHeader(wsEnclosed)
    
    With wsEnclosed.PageSetup
        .CenterHeader = "ENCLOSED JOBS"
    End With
End Sub

' Sort our FilteredData worksheet by DATE and VOLUME
Sub SortDateAndCount()
    Dim ws As Worksheet
    Dim col As Range
    Dim lastRow As Long
    ...
    With ws.Sort
        .SortFields.Clear
        .SortFields.Add Key:=ws.Columns(5), Order:=xlAscending
        .SetRange sortRange
        .Header = xlYes
        .MatchCase = False
        .Orientation = xlTopToBottom
...
' Sort our Kanbans worksheet by Stock_Location
Sub SortByKanbans()
    Dim ws As Worksheet
    Dim col As Range
    Dim lastRow As Long
    Dim wsKanbans As Worksheet

    On Error Resume Next
    Set ws = ThisWorkbook.Sheets("Kanbans")
    On Error GoTo 0
    
    If ws Is Nothing Then Exit Sub


    Set col = ws.Rows(1).Find("OUTER")
    If col Is Nothing Then
        MsgBox "Column OUTER not found.", vbExclamation
        Exit Sub
    End If
    ...
    Dim sortRange As Range
    Set sortRange = ws.Range(ws.Cells(1, 1), ws.Cells(lastRow, 7)) ' Col A to G, unmerged
    
    With ws.Sort
        .SortFields.Clear
        .SortFields.Add Key:=ws.Columns(4), Order:=xlAscending
        .SetRange sortRange
        .Header = xlYes
        .MatchCase = False
        .Orientation = xlTopToBottom
    ...
    Set wsKanbans = ThisWorkbook.Sheets("Kanbans")
    Call AddTimestampToHeader(wsKanbans)
    
    With wsKanbans.PageSetup
        .CenterHeader = "OUTERS SUMMARY"
    End With
End Sub

' Sort our Kanbans worksheet by Outers
Sub SortByOuters()
    Dim ws As Worksheet
    Dim col As Range
    Dim lastRow As Long
